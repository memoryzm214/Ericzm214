...
if cnt:
    note('表格格式', '全文表格', '表内字号混用：' + '、'.join(f'{int(k)/2:g}pt×{v}处' for k, v in sorted(cnt.items())),
         f'统一为{int(SZ)/2:g}pt（小四）', '按你的要求改回原始报告所用的12pt；上一轮曾统一为9pt，本轮已全部回改。')
lib.save(d)
