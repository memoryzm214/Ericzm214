...
def templates(d):
    ...
    return {
        'h1':   find(lambda p: p['style']=='2' and re.match(r'^5\S*研究总结$', p['text'].strip())),
        'h2':   find(lambda p: p['style']=='2' and p['text'].strip().startswith('5.1') and '成果概述' in p['text']),
        'body': find(lambda p: p['style']=='4' and p['text'].startswith('本研究聚焦于化解高校共享道路')),
        'plain':find(lambda p: p['text'].strip().startswith('图1') and '技术路线图' in p['text']),
    }
